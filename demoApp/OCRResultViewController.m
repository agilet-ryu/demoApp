...
@interface OCRResultViewController ()<UIWebViewDelegate, cameraScanManagerDelegate, faceIDManagerDelegate>
...
- (void)initScrollView{
    ...
    NSURL *locationURL = [NSURL URLWithString:self.currentModel.htmlPath];
...
- (void)didChecked:(UIButton *)button{
    ...
    self.cameraButton.backgroundColor = button.isSelected ? [UIColor colorWithHexString:[UITool shareUITool].baseColorHexString alpha:0.3f] : [UIColor whiteColor];
    self.cameraButton.layer.borderColor = button.isSelected ? [UIColor clearColor].CGColor : [UIColor colorWithHexString:[UITool shareUITool].baseColorHexString alpha:1.0f].CGColor;
    self.cameraButton.userInteractionEnabled = !button.isSelected;
    button.isSelected ? [self.cameraButton setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal] : [self.cameraButton setTitleColor:[UIColor colorWithHexString:[UITool shareUITool].baseColorHexString alpha:1.0f] forState:UIControlStateNormal];
    
    self.nextBT.backgroundColor = button.selected ? [UIColor colorWithHexString:[UITool shareUITool].baseColorHexString alpha:1.0f] : [UIColor colorWithHexString:[UITool shareUITool].baseColorHexString alpha:0.3f];
...
- (void)goNextView {
    ...
    [manager getBizTokenWithImage:self.currentModel.frontImage viewController:self];
}
...
- (NSDictionary *)getParamDic{
    if ([self.currentModel.buttonTitle isEqualToString:@"運転免許証"]) {
        return @{@"dl-name" : @"東京太郎",
                 @"dl-birth" : @"昭和５１年9月１７日",
                 @"dl-addr" : @"東京都町田市鶴見",
    ...
    } else if ([self.currentModel.buttonTitle isEqualToString:@"マイナンバーカード"]) {
        return @{
                 @"cardinfo-name": @"日本　花子",
    ...
    } else if ([self.currentModel.buttonTitle isEqualToString:@"パスポート"]) {
        return @{
                 @"ep-type": @"P<",
